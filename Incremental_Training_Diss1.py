...
# For each class, compute the centroid of features obtained from a few samples.
...
# Compute a matrix of Euclidean distances between class centroids.
...
# Greedily generate a class order by starting from an arbitrary class and 
# repeatedly choosing the next class that is most dissimilar (farthest)
...
def generate_class_order(dist_matrix):
    ...
    order = []
    current = remaining.pop(0)  # Start with the first class (could be randomized)
...
# Group indices by class, shuffle within each group, and then interleave them 
# according to the provided class_order.
...
# Compute centroids for each class (using 10 samples per class by default)
...
# Generate the dissimilar order
dissimilar_order = generate_class_order(dist_matrix)

print("\nTraining with Dissimilar Order")

incremental_acc_alpha = train_incrementally(
    net, train_data, val_data, dissimilar_order, criterion, optimizer, scaler, device, step_size=20, augmentation_fn=None
)
