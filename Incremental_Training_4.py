...
def validate(net, val_loader, criterion, device):
    ...
    num_classes = len(val_loader.dataset.dataset.classes)  # The base dataset for the subset
...
def train_incrementally(net, train_data, val_loader, order, criterion, optimizer, scaler, device, step_size=50, augmentation_fn=None):
    ...
    for step in range(step_size, len(train_data.classes) + 1, step_size):
        ...
        # For step size 50, we now run a constant 12 epochs per step
        current_epochs = 12
        ...
        new_data_indices = [i for i, (_, label) in enumerate(train_data.samples) if label in new_classes]
        combined_dataset = CombinedDataset(train_data, new_data_indices, replay_buffer)
        
        train_sampler_local = DistributedSampler(combined_dataset, shuffle=True)
        train_loader_step = DataLoader(combined_dataset, batch_size=batch_size, sampler=train_sampler_local,
                                       num_workers=8, pin_memory=True, drop_last=True)
        
        normal_epoch_samples_fixed = len(train_data) / dist.get_world_size()
        ...
        for epoch in range(current_epochs):
            ...
            train_loss, train_acc, epoch_samples = train_epoch(net, train_loader_step, criterion,
                                                               optimizer, scaler, device, augmentation_fn,
                                                               ewc_old_params=old_params, lambda_reg=lambda_reg)
            current_val_indices = [i for i, (_, label) in enumerate(val_data.samples) if label in current_classes]
            current_val_subset = Subset(val_data, current_val_indices)
            current_val_loader_local = DataLoader(current_val_subset, batch_size=batch_size, shuffle=False,
                                                  num_workers=8, pin_memory=True, drop_last=True)
            val_loss, val_acc = validate(net, current_val_loader_local, criterion, device)
            samples_this_step += epoch_samples
            samples_so_far += epoch_samples
            while samples_so_far >= normal_epoch_samples_fixed:
                scheduler.step()
                samples_so_far -= normal_epoch_samples_fixed
            if dist.get_rank() == 0:
        ...
        for cls in new_classes:
            cls_indices = [i for i, (_, label) in enumerate(train_data.samples) if label == cls]
...
incremental_acc_alpha = train_incrementally(
    net, train_data, val_loader, alphabetical_order, criterion, optimizer, scaler, device, step_size=50, augmentation_fn=None
)
