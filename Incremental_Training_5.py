...
def train_incrementally(net, train_data, val_data, order, criterion, optimizer, scaler, device, step_size=50, augmentation_fn=None):
    ...
    for step in range(step_size, len(train_data.classes) + 1, step_size):
        ...
        for epoch in range(current_epochs):
            ...
            # Validation
            current_val_indices = [i for i, (_, label) in enumerate(val_data.samples) 
                                 if label in current_classes]
            current_val_subset = Subset(val_data, current_val_indices)
            current_val_loader = DataLoader(current_val_subset, batch_size=batch_size, 
...
incremental_acc_alpha = train_incrementally(
    net, train_data, val_data, alphabetical_order, criterion, optimizer, scaler, device, step_size=50, augmentation_fn=None
)
